...
export async function GET() {
  ...
  try {
    ...
    const accountRes = await fetch(
      `${baseUrl}/${version}/${accountId}?fields=name,account_status&access_token=${accessToken}`
    );
      ...
      pagesList.data.map(async (page: FacebookPage) => {
        ...
        try {
          const detailRes = await fetch(
            `${baseUrl}/${version}/${page.id}?fields=fan_count&access_token=${page.access_token}`
          );
          ...
          return {
            ...
            access_token: page.access_token // Solo para debug
