...
export async function POST(request: NextRequest) {  
  try {
    ...
    const conversationsResponse = await fetch(
      `https://graph.facebook.com/v18.0/${pageId}/conversations?fields=id,participants,messages.limit(50){message,from,to,created_time,id,sticker,attachments}&limit=25&access_token=${accessToken}`,
    ...
    try {
      const sentMessagesResponse = await fetch(
        `https://graph.facebook.com/v18.0/${pageId}/messages?fields=message,from,to,created_time,id,sticker,attachments&limit=100&access_token=${accessToken}`
      );
         ...
         if (messages.length === 0) {
           try {
             const messagesResponse = await fetch(
               `https://graph.facebook.com/v18.0/${conversation.id}/messages?fields=message,from,to,created_time,id,sticker,attachments&limit=50&access_token=${accessToken}`
             );
