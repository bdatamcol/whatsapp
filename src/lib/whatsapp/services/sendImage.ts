...
function decryptData(encryptedText: string, secretKey: string): string {
  try {
    ...
    if (!secretKey || secretKey.length !== 64 || !/^[0-9a-fA-F]+$/.test(secretKey)) {
      console.warn('[sendImage] Clave de cifrado inválida o con formato incorrecto');
      return encryptedText;
    }
    ...
    console.warn('[sendImage] Error al descifrar datos, usando versión sin descifrar:', error);
    return encryptedText;
  }
}
